...
OUT = Path(__file__).resolve().parent                 # CSV artefacts -> "Ridge and LASSO/"
...
for path, alphas, sel, title, fname in [
    ...
    fig.tight_layout()
    fig.savefig(f"{IMAGE_DIR}/{fname}", dpi=150)
    plt.close(fig)
...
fig.tight_layout()
fig.savefig(f"{IMAGE_DIR}/cv_error_curves.png", dpi=150)
plt.close(fig)
...
ax.legend()
fig.tight_layout()
fig.savefig(f"{IMAGE_DIR}/coef_comparison.png", dpi=150)
plt.close(fig)
