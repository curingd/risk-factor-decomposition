...
REPO_ROOT = Path(__file__).resolve().parent.parent    # "Model Comparison/" -> repo root
FACTOR_CSV = REPO_ROOT / "Excel" / "factor_returns.csv"
RAW_COEF_CSV = REPO_ROOT / "Ridge and LASSO" / "ridge_lasso_raw_coefficients.csv"
OUT = Path(__file__).resolve().parent                 # CSV artefact -> "Model Comparison/"
IMAGE_DIR = REPO_ROOT / "Images"                      # figure -> Images/
...
rl = pd.read_csv(RAW_COEF_CSV, index_col=0)
...
plt.tight_layout()
fig.savefig(IMAGE_DIR / "model_fit_table.png", dpi=200, bbox_inches="tight",
            facecolor="white")
